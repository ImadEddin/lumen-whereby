...
class WherebyRequest {
    ...
    public function __construct($requestType, $endpoint, $accessToken, $baseUrl, $apiPrefix) {
        ...
        if (!$this->accessToken) {
            throw new WherebyException(config('whereby.errorAccessToken'));
        }
    ...
    public function execute($client = NULL) {
        ...
        try {
            $result = $client->request(
                    $this->requestType, $this->getRequestUrl(), [
            ...
            $error = new WherebyResponse($result);
        } catch (RequestException $e) {
            $result = $e->getResponse();
            $error = new WherebyResponse($result);
        }

        return (new WherebyResponse($result))->getBody();
    }
     ...
     *
     * @return \Whereby\Http\WherebyRequest
     *   WherebyRequest object.
     */
